...
const PPTConfiguratorProductScreen = (ProductScreen) =>
    class extends ProductScreen {
        setup() {
            super.setup();
            this.product_configurator_popup = true;
        }

        async _clickProductTemplate(event) {
            if (this.product_configurator_popup) {
                var product = event.detail;
                var ptav = Array.from(
                    new Set(
                        product.template.product_template_attribute_value_ids.map((x) =>
                ...
                var product_attribute_by_id = this.env.pos.db.product_attribute_by_id;
                var product_attribute_value_by_id =
                    this.env.pos.db.product_attribute_value_by_id;
...
const PPTProductItemConfigurator = (ProductItem) =>
    class extends ProductItem {
        setup() {
            super.setup();
        }
        async onClick() {
            this.trigger("click-product-template", this.props.product);
        }
    };

Registries.Component.extend(ProductItem, PPTProductItemConfigurator);